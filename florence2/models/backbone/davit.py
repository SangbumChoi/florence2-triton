    ...
    def __init__(
        ...
        dpr = [
            ...
            for x in torch.linspace(
                0, drop_path_rate, 2 * len(list(itertools.chain(*self.architecture)))
            )
        ]
        assert (
            self.num_stages
                    ...
                    MySequential(
                        *[
                            ChannelBlock(
                                ...
                                qkv_bias=qkv_bias,
                                drop_path=dpr[
                                    2 * (layer_id + layer_offset_id) + attention_id
                                ],
                                norm_layer=nn.LayerNorm,
                                ffn=ffn,
                            )
    ...
    def forward(self, x):
        ...
        for block_index, block_param in enumerate(self.architecture):
            ...
            for layer_index, branch_id in enumerate(block_param):
                features[branch_id], _ = self.main_blocks[block_index][layer_index](
                    features[branch_id], sizes[branch_id]
                )
...
def _create_transformer(variant, pretrained=False, default_cfg=None, **kwargs):
